                                                ...
                                                const std::vector<int>& fixed_indices) const
{
  ...
  const std::vector<trajopt_ifopt::JointPosition::ConstPtr> vars(&problem.vars[static_cast<std::size_t>(start_index)],
  ...
  if (collision_cost_config->type != tesseract_collision::CollisionEvaluatorType::NONE)
    addCollisionSquaredCost(problem.nlp, vars, problem.environment, manip_info, collision_cost_config, fixed_indices);
